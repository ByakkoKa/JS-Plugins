...
window.onload = function() {
    ...
    function showLightbox(element, data) {
        if(!clicked) {
            ...
            // Previnir seleção, para não ocorrer erros de slide
    ...
    function setGallery(element, data){
        ...
        if(!isMobile ) showControlers(true);
        isGallery = true;
        thumbs = [];

        // Verifica se é uma galeria oculta ou não
        if(data['galleryName'] != 'mylightbox[folder]') {
            let thumbsClone = [].slice.call(document.querySelectorAll(`[rel="${data['galleryName']}"]`));
    ...
    function showGallery(data) {
        ...
        let height = gallery.scrollHeight;
    ...
    function closeLb(element) {
        ...
        if(isGallery) {
            if(!isMobile ) showControlers(false);
            isGallery = false;
        }
    }
